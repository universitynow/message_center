class MailboxerNamespacingCompatibility < ActiveRecord::Migration
  ...
  def self.up
    ...
    MessageCenter::Notification.where(type: 'Message').update_all(type: 'Mailboxer::Message')
  end
  ...
  def self.down
    ...
    MessageCenter::Notification.where(type: 'Mailboxer::Message').update_all(type: 'Message')
  end
end
